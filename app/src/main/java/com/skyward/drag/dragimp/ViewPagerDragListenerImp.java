...
public class ViewPagerDragListenerImp extends DragListenerDispatcher<ViewPager, DragInfo> {
    ...
    private DragListenerDispatcher mLastDragListener;
...
//        //item
    ...
    public void onDragEnd(DragInfo dragInfo, ViewPager viewPager) {
        ...
        if (mDragging) {
            mDragging = false;
            if (mLastDragListener != null) {
                mLastDragListener.onDragExit(dragInfo, getPageView(dragInfo.pageIndex, viewPager));
            }
            clearScrollRunnable();
        }
    ...
    @Override
    public void onDragLocation(DragInfo dragInfo, final ViewPager viewPager) {
    ...
    private void checkTouchMove(final DragInfo dragInfo, ViewPager viewPager) {
        ...
        if (dragListener != null) {
            if (mLastDragListener != dragListener) {
                ...
                if (mLastDragListener != null) {//(mLastDragLisener == null) is first drag, first drag do nothing
                    mLastDragListener.onDragExit(dragInfo, pageView);
                    ...
                    mLastDragListener.onPageTransfer(dragInfo, newDragInfo);
                    //end page transfer
            ...
            //on hover
            dragListener.onDragLocation(dragInfo, pageView);
        } else {
            if (mLastDragListener != null) {
                mLastDragListener.onDragExit(dragInfo, pageView);
            }
        }
        mLastDragListener = dragListener;
    }
