package com.skyward.drag.adapter;
...
import com.skyward.drag.DragManager;
import com.skyward.drag.DragNotifier;
import com.skyward.drag.dragimp.RecyclerDragListenerImp;
import com.skyward.drag.shadowbuilder.NoForegroundShadowBuilder;
...
public abstract class BaseDragPageAdapter<V> extends BaseGridPagerAdapter<V> implements View.OnLongClickListener {
    ...
    public BaseDragPageAdapter(Context context, int row, int column, List<V> list, DragListenerDispatcher<ViewPager, DragInfo> dragListenerDispatcher) {
        super(context, row, column, list);

        mDragManager = new MyDragManager();
        dragListenerDispatcher.attachDragManager(mDragManager);
